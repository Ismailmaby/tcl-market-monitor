...
def get_prompt():
    ...
    return (
        ...
        "]}\n"
        "Fill with real insights about UAE/Saudi hotel market, Brazil LATAM hospitality, "
        "Samsung LYNK vs LG ProCentric, IPTV middleware, TCL BD opportunities."
    )
...
def call_api(prompt, api_key):
    ...
    with urllib.request.urlopen(req, timeout=90) as resp:
        ...
        print("  Raw preview: " + raw[:500])
        return raw

def extract_sections(raw):
    # Try parsing raw as JSON first (might be the full API response)
    try:
        data = json.loads(raw)
        ...
        if "content" in data:
            ...
            if start >= 0 and end > start:
                sections_data = json.loads(text[start:end])
                return sections_data.get("sections", [])
        # Maybe raw itself has sections
        if "sections" in data:
            return data["sections"]
    except Exception as e:
        print("  Parse attempt 1 failed: " + str(e)[:80])

    # Try finding JSON directly in raw
    try:
        start = raw.find('{"sections"')
        if start >= 0:
            end = raw.rfind("}") + 1
            sections_data = json.loads(raw[start:end])
            return sections_data.get("sections", [])
    except Exception as e:
        print("  Parse attempt 2 failed: " + str(e)[:80])

    print("  All parse attempts failed")
    print("  Full raw: " + raw[:1000])
    return []
