...
def get_prompt():
    ...
    return (
        ...
        "Generate a daily briefing with REAL insights based on your knowledge of the hospitality industry.\n"
...
def call_api_with_search(prompt, api_key):
    payload = json.dumps({
        "model": MODEL,
        "max_tokens": 2000,
    ...
    try:
        with urllib.request.urlopen(req, timeout=90) as resp:
            ...
            print("  [Search] Response length: " + str(len(raw)))
            return json.loads(raw), None
    except urllib.error.HTTPError as e:
        err = e.read().decode()[:300]
        print("  [Search] HTTP " + str(e.code) + ": " + err)
        return None, err
    except Exception as e:
        print("  [Search] Error: " + str(e)[:100])
        return None, str(e)

def call_api_no_search(prompt, api_key):
    payload = json.dumps({
        "model": MODEL,
        "max_tokens": 1500,
        "messages": [{"role": "user", "content": prompt}]
    ...
    try:
        with urllib.request.urlopen(req, timeout=60) as resp:
            raw = resp.read().decode()
            print("  [NoSearch] Response length: " + str(len(raw)))
            return json.loads(raw), None
    except urllib.error.HTTPError as e:
        err = e.read().decode()[:300]
        print("  [NoSearch] HTTP " + str(e.code) + ": " + err)
        return None, err
    except Exception as e:
        print("  [NoSearch] Error: " + str(e)[:100])
        return None, str(e)
...
def extract_sections(response):
    ...
    if start == -1 or end == 0:
        print("  [WARN] No JSON found. Preview: " + text[:200])
        return []
    try:
        ...
        print("  JSON preview: " + text[start:start+200])
        return []
...
def main():
    ...
    print("Trying with web search...")
    response, err = call_api_with_search(prompt, ANTHROPIC_KEY)

    if response is None:
        print("Web search failed, trying without...")
        response, err = call_api_no_search(prompt, ANTHROPIC_KEY)

    if response is None:
        print("Both failed. Error: " + str(err))
        sections = []
    else:
